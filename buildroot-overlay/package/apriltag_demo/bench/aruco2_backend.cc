...
class Aruco2Backend final : public Backend {
public:
    explicit Aruco2Backend(const BenchmarkConfig& config)
    ...
    BackendKind kind() const override { return BackendKind::Aruco2; }
    ...
    DetectionResult detect(const PreparedImage& image) override
    {
        ...
        const auto markers = cv::aruco2::detectFiducialMarkers(
            detector_input, cv::aruco2::DICT_APRILTAG_36h11, parameters_);
        ...
        for (const auto& marker : markers) {
            if (marker.corners.size() != 4 || marker.id < 0) {
            ...
            for (int corner = 0; corner < 4; ++corner) {
                normalized.corners[corner * 2] =
                    marker.corners[corner].x * factor_;
                normalized.corners[corner * 2 + 1] =
                    marker.corners[corner].y * factor_;
...
private:
    static int checked_detection_count(std::size_t count)
    {
        if (count > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
            throw std::runtime_error("ArUco2 detection count overflows int");
        }
    ...
    static cv::Mat image_view(const PreparedImage& image)
    {
        if (image.width > static_cast<std::size_t>(std::numeric_limits<int>::max()) ||
            ...
            throw std::runtime_error("image geometry exceeds ArUco2 limits");
        }
...
{
    return std::unique_ptr<Backend>(new Aruco2Backend(config));
}
