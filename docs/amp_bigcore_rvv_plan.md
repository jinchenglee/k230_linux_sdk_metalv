...
> **Provenance:** the body of this document was written on the
> `opt_linux_on_small_core` / `..._cherry-picked` branch and describes the
> state of *that* branch. It is reproduced here on `dev` for reference.
> Wherever the text says "this branch", read it as the small-core branch,
> **not** `dev`. See the scope note immediately below, and
> `docs/k230_amp_payload_slots.md` for the
> generalized goal this feeds into.
...
> ## Scope: this is ONE AMP scenario, not the only one
>
> This document describes a single, specific role assignment:
> **Linux on the small core + bare-metal RVV payload on the big core.**
> It is not a statement that AMP on K230 must look like this.
>
> The intended long-term architecture is a **configurable payload slot per
> core**: "what runs on core N" is a build-time selection (Linux, RT-Smart,
> bare-metal payload, or nothing), chosen independently for each core. This
> plan is then just one of the possible slot assignments.
>
> In particular, note that the **opposite** assignment is what the `dev`
> branch actually ships today: `k230_canmv_defconfig` and
> `k230_canmv_01studio_defconfig` both run **Linux on the big core** (all of
> userspace is built `-mcpu=c908v -mrvv-auto-vectorize`, plus the RVV-only
> libnncase / AI2D_KPU blobs, so they cannot execute on a non-vector core).
> Reading this document without that context has already caused confusion
> about which core "the" Linux runs on. There is no single answer -- it is a
> configuration choice.
>
> Two premises below are inherited from this branch's setup and should be
> re-verified before being relied on elsewhere; see the inline notes marked
> **[premise]**.
...
> **Confirmed, but the hart numbering in this table is backwards.**
>
> K230 has exactly one RVV-capable core -- the big core; the small core has
> no vector unit. That part is correct, and the constraint is hard: a slot on
> the small core cannot execute vector code, which is why the AI/NN stack
> (libnncase, AI2D_KPU, face_detect, ai_demo) is disabled here.
>
> However, **"CPU0"/"CPU1" in K230 documentation are SoC labels, not
> architectural hart IDs, and the two are inverted.** OpenSBI on `dev`
> reports `Boot HART ID : 0` together with a vector-capable ISA, read from
> `mhartid` in M-mode before any DT is consulted. So the **big RVV core is
> architectural hart 0** and the small core is hart 1 -- the opposite of what
> the SoC labels suggest. The reset/clock registers use the SoC labels
> (`cpu1_hart_rstvec` = big core), so any code mapping hartid to register set
> must invert. See the table in
> `docs/k230_amp_payload_slots.md`.
>
> **This branch does genuinely redirect Linux to the small core.** The
> mechanism is commit `1e02ce1` ("Try to bringup linux on small core only",
> 2025-10-14), which sets `CONFIG_LINUX_RUN_CORE_ID = 0` in
> `board/canaan/common/sdk_autoconf.h`. That compiles out the
> `de_reset_big_core()` + `wfi`-park block in
> `board/canaan/common/k230_img.c:270`, so SPL falls through to
> `uboot(0, dtb)` and stays on the small core instead of handing U-Boot proper
> to the big one. It requires U-Boot to have been rebuilt to take effect.
>
> **What the `reg` numbers here mean is still open.** The big core is
> architectural hart 0 (OpenSBI reports `Boot HART ID : 0` with a vector ISA
> on `dev`). The small core's `mhartid` has *not* been established -- a
> big-core log says nothing about it, and K230's two CPU subsystems are not a
> conventional SMP pair, so each may present as hart 0 when independently
> booted. Read `Boot HART ID` and `Boot HART Base ISA` from this branch's own
> OpenSBI banner to settle it. That answer also decides whether a single
> OpenSBI can manage both cores at all -- see the "Hart ID space" section of
> `docs/k230_amp_payload_slots.md`.
...
> **[premise] This relies on an accident, not a mechanism.** CPU1 stays
> `STOPPED` here because Linux refuses a hart whose ISA does not match the
> boot hart. That is incidental to the heterogeneous ISA strings above; it is
> not a supported way to reserve a core. The deliberate equivalents are
> `cpu@1 { status = "disabled"; }` or `maxcpus=1` (see "First-cut
> implementation order", step 1), and under a domain-based firmware setup
> hart ownership becomes an explicit declaration instead.
...
> **Status note:** the two commits described below are now **on `dev`**
> (`buildroot-overlay/linux/0023-add-gdma-vo-rotation.patch` and
> `0019-dts-add-nonai2d.patch`). So the "no DT node, no driver, CPU1 just
> takes GSDMA" premise no longer holds against `dev`, and step 6 of the
> implementation order cannot be forward-ported as written. Of the three
> options listed at the end of this section, the one that fits the
> configurable-slot goal is the third: treat **device ownership as per-slot
> configuration** and statically split GSDMA's 8 channels between the slots,
> rather than reverting the VO rotation feature or arbitrating at runtime.
